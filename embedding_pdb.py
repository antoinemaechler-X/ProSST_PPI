...
def parse_pdb_chain_sequence(pdb_file: str, chain_id: str = 'A') -> str:
    """Parse PDB file and return amino acid sequence for specified chain."""
    ...
    # Get all chains with IDs
    chains = [chain for chain in structure.get_chains() if chain.id != ' ']
    if not chains:
        raise ValueError(f"No valid chains found in {pdb_file}")
    
    # If chain_id is 'A', take the first chain; if 'B', take the second
    target_chain = chains[0] if chain_id == 'A' else chains[1] if len(chains) > 1 else None
    
    if target_chain is None:
        raise ValueError(f"Could not find chain {chain_id} in {pdb_file}")
...
def main():
    ...
    token_dir = "data/SKEMPI2/SKEMPI2_cache/token_wildtype_evoef1_2048"
    pdb_dir = "data/SKEMPI2/SKEMPI2_cache/wildtype_evoef1"
    output_dir = "data/SKEMPI2/SKEMPI2_cache/embedding_wildtype_evoef1_2048"
    ...
    for i, fasta_file in enumerate(fasta_files, 1):
        # Extract PDB ID and chain from filename (format: 0_1CSE_A.fasta)
        prefix = fasta_file.stem.split('_')[0]  # Get 0 from 0_1CSE_A
        pdb_id = fasta_file.stem.split('_')[1]  # Get 1CSE from 0_1CSE_A
        chain_id = fasta_file.stem.split('_')[2]  # Get A from 0_1CSE_A
        
        # Check if output file already exists
        output_path = Path(output_dir) / f"{fasta_file.stem}_embeddings.npy"
        if output_path.exists():
            #print(f"\nSkipping {fasta_file.name} - embeddings already exist")
            continue
        
        print(f"\nProcessing file {i}/{total_files}: {fasta_file.name}")
        
        # Get corresponding PDB file with the correct prefix
        ...
        try:
            ...
            print(f"Reading PDB file {pdb_file} for chain {chain_id}...")
            sequence = parse_pdb_chain_sequence(str(pdb_file), chain_id=chain_id)
            if sequence is None:
                print(f"Skipping {fasta_file.name} - Could not find chain {chain_id}")
                continue
            
            # Get structure sequence from FASTA
            print("Reading structure sequence...")
